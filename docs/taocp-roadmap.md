...
| Subtractive GCD | Euclid (Elements VII) | `subtractive-gcd` | ✅ subtraction-only = Nat.gcd |
...
| Merge two sorted runs | Vol 3 §5.2.4 | `merge-sorted-correct` | ✅ sorted ∧ permutation of l₁++l₂ |
| XOR swap | folklore | `xor-swap` | ✅ swap with no temporary |
| Tail-recursive reversal | folklore | `rev-append-correct` | ✅ fast_rev = rev (linear) |
| Binary increment | Vol 4A | `binary-increment-correct` | ✅ inc adds exactly one |
